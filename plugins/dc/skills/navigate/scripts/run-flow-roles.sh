...
# --- Validate auth files exist (fail fast) ---

AUTH_DIR=".agents.tmp/.auth"
MISSING=0
for ROLE in "${ROLES[@]}"; do
  AUTH_FILE="${AUTH_DIR}/${APP}-${ROLE}.json"
  if [[ ! -f "$AUTH_FILE" ]]; then
    echo "Error: Auth file not found: ${AUTH_FILE}"
    echo "  Export it first: npx tsx plugins/dc/skills/browser-login/scripts/export-auth.ts ${APP} ${ROLE}"
    MISSING=1
  fi
done
